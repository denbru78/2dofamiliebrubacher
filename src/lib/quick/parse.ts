/* Regelbasierter Parser für die Schnelleingabe – ohne KI.
   Reihenfolge (Vorranglogik der Master-Datei):
   1 normalisieren · 2 Personen · 3 Datum/Zeit · 4 Priorität · 5 starke Aktionen
   6 Kategorie (gewichtet) · 7 Metadaten aus Titel entfernen · 8 Verb normalisieren · 9 Titel säubern */
import type { DueKind, Priority, Profile } from '../types'
import { addDays, toISODate } from '../dates'
import { CATEGORY_WORDS, DUE_BUCKETS, DUE_RELATIVE, PRIORITY_PHRASES, TITLE_NOISE, WEEKDAYS, type CatWord } from './dictionary'
...
export interface ParseResult {
  ...
  category: string
  priority: Priority
  notes: string[] // Hinweise für die Vorschau ("Zeit nicht eindeutig erkannt")
  matched: { persons: string[]; due: string[]; time: string[]; priority: string[]; category: string[] }
}
...
function findWord(text: string, word: string, wholeWord = false): { index: number; length: number } | null {
...
export function parseQuick(
  ...
  const notes: string[] = []
  const matched: ParseResult['matched'] = { persons: [], due: [], time: [], priority: [], category: [] }
  const spans: { index: number; length: number }[] = [] // zu entfernende Bereiche (im normalisierten Text)

  const cut = (index: number, length: number) => {
    // Überlappende Bereiche nur einmal entfernen (z. B. „Mama“ als Name und als Rollenwort)
    if (spans.some((sp) => index < sp.index + sp.length && sp.index < index + length)) return
  ...
  const byAvatar = (file: string) => profiles.find((p) => p.avatar === `/avatars/${file}`)
  const byName = (n: string) => profiles.find((p) => normalize(p.display_name) === normalize(n))
  const aliases: { word: string; ids: string[] }[] = []
  for (const p of profiles) {
    aliases.push({ word: p.display_name, ids: [p.id] })
    const first = p.display_name.split(/\s+/)[0]
    if (first && first !== p.display_name) aliases.push({ word: first, ids: [p.id] })
  }
  ...
  if (papa) aliases.push({ word: 'papa', ids: [papa.id] }, { word: 'vater', ids: [papa.id] })
  if (mama) aliases.push({ word: 'mama', ids: [mama.id] }, { word: 'mutter', ids: [mama.id] })
  if (tochter) aliases.push({ word: 'tochter', ids: [tochter.id] })
  if (sohn) aliases.push({ word: 'sohn', ids: [sohn.id] })
  if (members.length) aliases.push({ word: 'die kinder', ids: members.map((p) => p.id) }, { word: 'beide kinder', ids: members.map((p) => p.id) }, { word: 'kinder', ids: members.map((p) => p.id) })
  if (admins.length) aliases.push({ word: 'mama und papa', ids: admins.map((p) => p.id) }, { word: 'papa und mama', ids: admins.map((p) => p.id) }, { word: 'eltern', ids: admins.map((p) => p.id) })
  if (me) aliases.push({ word: 'ich', ids: [me.id] })
  for (const k of extra) if (k.type === 'person') aliases.push({ word: k.word, ids: [k.value] })
  ...
  const objectRanges: { index: number; length: number }[] = []
  for (const a of aliases) {
    ...
    if (!hit) continue
    if (objectRanges.some((r) => hit.index >= r.index && hit.index < r.index + r.length)) continue
    ...
    if (!objectOnly) a.ids.forEach((id) => ids.add(id))
    matched.persons.push(a.word)
    if (objectOnly) {
      objectRanges.push({ index: hit.index, length: hit.length })
      continue
    }
    let len = hit.length
    ...
    if (m1 && !/^(\s+und)$/.test(m1[0])) len += m1[0].length
  ...
  for (const w of ['einer von uns', 'kann jemand', 'wir müssen', 'wir sollten', 'wir sollen', 'wer kann', 'wer macht', 'irgendwer', 'jemand', 'wir']) {
  ...
  const timeRules: [RegExp, (m: RegExpExecArray) => string | null][] = [
    ...
    [/\b(\d{1,2}|[a-zäöü]+) (nach|vor) (\d{1,2}|[a-zäöü]+)\b/, (m) => {
      const mins = NUMWORDS[m[1]] ?? Number(m[1]); const h = NUMWORDS[m[3]] ?? Number(m[3])
      if (!Number.isFinite(mins) || !Number.isFinite(h) || mins > 59) return null
  ...
  const buckets: [string, string][] = [['morgens', '09:00'], ['früh', '09:00'], ['vormittags', '10:00'], ['mittags', '12:00'], ['nachmittags', '15:00'], ['abends', '19:00'], ['abend', '19:00']]
  for (const [w, t] of buckets) {
    const hit = findWord(text, w, true)
    if (hit && !/heute|morgen/.test(text.slice(Math.max(0, hit.index - 7), hit.index))) {
      if (!time) time = t
      else if ((w === 'abends' || w === 'abend' || w === 'nachmittags') && Number(time.slice(0, 2)) < 12) eveningShift = true
      cut(hit.index, hit.length)
      matched.time.push(w)
      break
    }
  }
  if (eveningShift && time) time = `${pad(Number(time.slice(0, 2)) + 12)}${time.slice(2)}`

  const setDate = (d: Date, kind: DueKind, label: string, span: { index: number; length: number }, phrase: string) => {
    due_kind = kind
  ...
  const rel = DUE_RELATIVE.slice().sort((a, b) => b[0].length - a[0].length)
  let dateFound = false
  for (const [w, v] of rel) {
    ...
    if (v === 'end_of_month') {
      ...
      if (!m) continue
      const d = addDays(today, Number(m[1]))
      const kind: DueKind = m[1] === '0' ? 'today' : m[1] === '1' ? 'tomorrow' : 'date'
      setDate(d, kind, m[1] === '0' ? 'Heute' : m[1] === '1' ? 'Morgen' : `Übermorgen (${weekdayLabel(d)})`, hit, w)
      if (m[2] && !time) time = m[2]
    }
    dateFound = true
    break
  }
  ...
  if (!dateFound) {
    const m1 = /(?<![\d.])(\d{1,2})\.(\d{1,2})\.(\d{2,4})?(?![\d])/.exec(text)
    ...
    if (mm) {
      ...
      if (!(m1 && m1[3]) && d < new Date(today.getFullYear(), today.getMonth(), today.getDate())) d = new Date(year + 1, month - 1, day, 12)
      if (d.getMonth() === month - 1) {
        const span = { index: mm.index, length: mm[0].length }
        const before = text.slice(0, mm.index)
        const am = /(\sam\s*)$/.exec(before)
        if (am) { span.index -= am[1].length; span.length += am[1].length }
  ...
  // Zeiträume (Buckets)
  if (!dateFound) {
    const bk = DUE_BUCKETS.slice().sort((a, b) => b[0].length - a[0].length)
    for (const [w, v] of bk) {
      const hit = findWord(text, w, true)
      if (!hit) continue
      const dow = (today.getDay() + 6) % 7 // Mo=0
      const monday = addDays(today, -dow)
      if (v === 'this_week' || v === 'end_this_week') { setDate(addDays(monday, 6), 'week', 'Diese Woche', hit, w) }
      else if (v === 'weekend') { setDate(addDays(monday, dow <= 5 ? 5 : dow), 'weekend', 'Wochenende', hit, w) }
      else if (v === 'next_weekend') { setDate(addDays(monday, 12), 'date', 'Nächstes Wochenende', hit, w) }
      else if (v === 'next_week') { setDate(addDays(monday, 13), 'date', 'Nächste Woche (bis So)', hit, w); notes.push('„nächste Woche“ als Frist bis Sonntag der nächsten Woche gesetzt') }
      else if (v === 'week_after_next') { setDate(addDays(monday, 20), 'date', 'Übernächste Woche', hit, w) }
      else if (v === 'this_month') { setDate(new Date(today.getFullYear(), today.getMonth() + 1, 0), 'date', 'Diesen Monat', hit, w) }
      else if (v === 'next_month') { setDate(new Date(today.getFullYear(), today.getMonth() + 2, 0), 'date', 'Nächsten Monat', hit, w) }
      else if (v === 'beginning_next_month') { setDate(new Date(today.getFullYear(), today.getMonth() + 1, 5), 'date', 'Anfang nächsten Monats', hit, w) }
      else if (v === 'middle_month') { setDate(new Date(today.getFullYear(), today.getMonth(), 15), 'date', 'Mitte des Monats', hit, w) }
      else continue
      dateFound = true
      break
    }
  ...
  if (!dateFound) {
    const wd = WEEKDAYS.slice().sort((a, b) => b[0].length - a[0].length)
    for (const [w, n] of wd) {
      ...
      let diff = n - cur
      if (diff <= 0) diff += 7
      if (/diesen/.test(m[0]) && n === cur) diff = 0
  ...
  if (!dateFound) {
    for (const w of ['irgendwann', 'bei gelegenheit', 'gelegentlich']) {
      const hit = findWord(text, w, true)
      if (hit) { due_kind = 'someday'; due_label = 'Irgendwann'; cut(hit.index, hit.length); matched.due.push(w); dateFound = true; break }
    }
  }
  if (!dateFound) {
    for (const w of ['später', 'demnächst', 'bald', 'nachher', 'anfang der woche', 'mitte der woche', 'nach dem essen', 'nach der schule', 'nach der arbeit']) {
      if (findWord(text, w, true)) { notes.push(`Zeit nicht eindeutig erkannt („${w}“) – bitte Termin wählen`); break }
    }
  }
  if (time && !dateFound) {
    // Uhrzeit ohne Datum → heute
    due_kind = 'today'; due_date = toISODate(today); due_label = 'Heute'
  }

  // ---- 4) Priorität (Negationen/längste zuerst) -----------------------
  let priority: Priority = 'none'
  const pr = PRIORITY_PHRASES.slice().sort((a, b) => b[0].length - a[0].length)
  const negated = /\bnicht (dringend|wichtig|eilig)\b/.exec(text)
  if (negated) { cut(negated.index, negated[0].length); matched.priority.push(negated[0]) }
  else {
    for (const [w, v] of pr) {
      if (w === 'muss' || w === 'muss noch' || w === 'irgendwann' || w === 'normal') continue // "muss" ist Titel-Noise, nicht Priorität (Beispiel 7 ausgenommen)
      const hit = findWord(text, w, true)
      if (!hit) continue
    ...
    if (priority === 'none' && matched.persons.includes('wir müssen')) priority = 'important'
  }

  // ---- 5/6) Kategorie ---------------------------------------------------
  const scores = new Map<string, number>()
  const has = (list: string[]) => list.some((w) => !!findWord(text, w))
  ...
  const add = (cat: string, n: number, w: string) => {
    if (!categoryNames.includes(cat)) return
    scores.set(cat, (scores.get(cat) ?? 0) + n)
    matched.category.push(`${w}→${cat}`)
  }
  const words: CatWord[] = [...CATEGORY_WORDS, ...extra.filter((k) => k.type === 'category').map((k) => ({ w: k.word, cat: k.value, weight: 3, whole: true }))]
  for (const cw of words) {
    if (cw.w === 'pool' && !poolObj) continue
    const hit = findWord(text, cw.w, cw.w.length <= 3 || !!cw.whole)
    if (!hit) continue
    if (cw.ctx === 'school' && !schoolCtx) continue
    if (cw.ctx === 'buy' && !buyMain) continue
    if (cw.ctx === 'car' && !carCtx) continue
    if (cw.ctx === 'child' && !childCtx) continue
    if (cw.ctx === 'garden' && !gardenCtx) continue
    if (cw.ctx === 'poolobj' && !poolObj) continue
    add(cw.cat, cw.w.includes(' ') ? 5 : cw.weight, cw.w)
  }
  // Hauptaktionsregeln
  if (buyMain) add('Besorgen & Kaufen', 6, 'Kaufverb')
  if (has(['ölstand', 'ölwechsel', 'tüv', 'inspektion', 'reifen', 'tanken', 'aussaugen', 'waschanlage', 'autowäsche'])) add('Auto & Mobilität', 5, 'Fahrzeugwartung')
  if (carCtx && has(['wasch', 'saug', 'reinig', 'putz', 'staubsaug'])) add('Auto & Mobilität', 2, 'Fahrzeug reinigen')
  if (has(SCHOOL_STRONG) && !buyMain) add('Schule', 5, 'Schulbegriff')
  if (has(['recherchier', 'vergleich', 'nachsehen', 'nachschau', 'googeln', 'rausfinden', 'herausfinden', 'ausmessen', 'suchen']) && !buyMain) add('Prüfen & Recherchieren', 4, 'Prüfverb')
  if (has(['prüf', 'check']) && !carCtx && !buyMain) add('Prüfen & Recherchieren', 3, 'Prüfverb')
  if (has(['anmelden', 'vereinbaren', 'termin machen', 'kündig', 'überweis', 'bezahl']) && !has(SCHOOL_STRONG)) add('Organisation', 3, 'Organisationsverb')
  if (has(['zum arzt', 'zum zahnarzt', 'zum kinderarzt', 'zahnarzttermin', 'arzttermin']) && childCtx) add('Familie & Kinder', 5, 'Kind+Arzt')
  if (has(['wegbring', 'entsorg', 'wertstoffhof', 'zurückschick'])) add('Organisation', 2, 'Entsorgung/Versand')
  if (has(['sperrmüll']) && has(['anmeld'])) add('Organisation', 3, 'Sperrmüll anmelden')
  // Personenwörter zählen nicht als Kategorie (Vorrang Personenerkennung)
  if (assignee_ids.length && !childCtx) scores.delete('Familie & Kinder')

  let category = 'Sonstiges'
  let best = 0
  for (const [c, s] of scores) if (s > best) { best = s; category = c }
  if (best < 1) category = categoryNames.includes('Sonstiges') ? 'Sonstiges' : (categoryNames[0] ?? 'Sonstiges')
  ...
  for (const s of sorted) t = t.slice(0, s.index) + ' ' + t.slice(s.index + s.length)
  const removeAll = (w: string) => {
  ...
  const noise = TITLE_NOISE.slice().sort((a, b) => b[0].length - a[0].length)
  for (const [w, v] of noise) if (v !== 'optional_remove') removeAll(w)
  for (const w of ['ganz', 'mal', 'noch', 'einfach', 'doch', 'eben', 'wenn du zeit hast', 'wenn zeit ist', 'wenn wir zeit haben', 'wenn möglich', 'ihr', 'du', 'dann']) removeAll(w)
  t = t.replace(/[,;:!?]+/g, ' ').replace(/\s+/g, ' ').trim()
  // Verwaiste Präpositionen nach dem Entfernen von Zeit/Person
  for (let i = 0; i < 3; i++) t = t.replace(/^(um|am|an|für|und|oder)\s+/i, '').replace(/\s+(und|oder|um|am)$/i, '').replace(/\s+(um|am)$/i, '').replace(/\s(um|am)\s(?=[a-zäöü])/gi, ' ')
  ...
  for (const [re, rep] of rewrites) {
    ...
    if (rei.test(t)) { t = t.replace(rei, rep); break }
  }
...
function capitalizeTitle(t: string): string {
  if (!t) return t
